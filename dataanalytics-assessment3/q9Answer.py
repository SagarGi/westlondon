...
print("\nBuilding and Evaluating Logistic Regression Classifier")
label_col = "Outcome"
all_feature_cols = [c for c in dataFlow3.columns if c != label_col and c not in ["CustomerID"]]
...
# Step 1: Index categorical features (e.g., 'Gender', 'PurchaseCategory')
...
# Step 2: Index the label column ('Outcome') if it's a string
...
# Step 5: Create Logistic Regression Classifier model
lr = LogisticRegression(labelCol="indexedLabel", featuresCol="features", maxIter=10, regParam=0.01)
...
# Using MulticlassClassificationEvaluator for general accuracy
...
# If 'Outcome' is strictly binary (0/1), BinaryClassificationEvaluator can also be used for AUC.
...
print(f"Logistic Regression Model Area Under ROC (AUC): {auc}")
print(f"\n\n")
